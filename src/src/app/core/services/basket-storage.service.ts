...
export class BasketStorage {
  ...
  public get basketItems$(): Observable<IReadonlyBasketItem[]> {
    return this.basketSource.asObservable().pipe(map(b => b.items));
  }

  public get basketTotals$(): Observable<IBasketTotals> {
    return this.basketSource.asObservable().pipe(map(this.basketToBasketTotals));
  }

  public get basketTotals(): IBasketTotals {
    return this.basketToBasketTotals(this.basket);
  }

  public get basketItems(): IReadonlyBasketItem[] {
    return this.basket.items;
  }

  private basketToBasketTotals(basket: Basket): IBasketTotals {
    const subtotal = basket.items.map(i => i.product.price * i.quantity).reduce((acc, cur) => acc + cur, 0);
    const shipping = 10;
